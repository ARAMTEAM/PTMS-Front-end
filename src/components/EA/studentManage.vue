<template>
    <div class="Container" >
      ...
      <div class="MForm">
        ...
        <Input search enter-button style="width:30%;float:right;" placeholder="检索学生" />
        <br><br>
          ...
          <Modal v-model="Modal" width="600" :closable="false" :mask-closable="false" >
            ...
            <Form v-if="Modal" ref="Form" :model="formItem" :label-width="100" :rules="ruleInline" >
                ...
                <FormItem label="学生密码" prop="studentPassword">
                  <Input type="password" password v-model="formItem.studentPassword" placeholder="更新时若不更改密码请不要填写"></Input>
                </FormItem>
...
export default {
  ...
  data(){
    return {
      ...
      formItem: {studentId:'',studentName:'',studentPassword:'',studentClass: '',studentTelephone: '',trainingId:0, },
      ruleInline: {
          studentId: [
          ...
          studentPassword: [
              { required: false,message:'学生密码禁止为空', trigger: 'blur' }
          ],
          studentClass: [
          ...
          trainingId: [
              { required: true,type:'number',message: '学生所处实训禁止为空', trigger: 'blur'  }
          ],
      },
    }
  },
  ...
  methods: {
      ...
      show (index) {
          this.$Modal.info({
              ...
              <br>学生电话：${this.data[index].studentTelephone}<br>所在实训：${this.data[index].trainingName}`
          })
      },
      //进入教师项目页面
      enter (index) {
      ...
      remove (index) {
        var result = confirm("您确认删除吗？")
          if(result){
            axios.delete(url+this.data[index].studentId)
            .then(res=>{
              if(res.data.success){
